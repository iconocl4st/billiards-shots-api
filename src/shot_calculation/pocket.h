...
namespace billiards::shots {
	...
	inline
	void tidy_pocket_posts(AssignmentStatus& status, std::shared_ptr<GoalPostTarget>& target) {
		...
		if (valid) {
			...
			ass.info.valid_calculations &= gp11_dist <= gp12_dist && gp22_dist <= gp21_dist;
		}
	}
	...
	bool maybe_assign_pocket_target(AssignmentStatus& status) {
		...
		const auto& pocket = ass.params.table.get_pocket(pocket_step->pocket);
		const auto& source = get_previous_status(status).exiting_source;
	...
	bool maybe_assign_pocket(AssignmentStatus& status) {
		const b1 = assign_pocket_source(status);
		const b2 = assign_pocket_target(status);
		return b1 || b2;
	}
}
